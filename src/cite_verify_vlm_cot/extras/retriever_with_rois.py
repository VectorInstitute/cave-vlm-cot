...
#     img_high = img_pil.resize((GDINO_RUN_SIZE, GDINO_RUN_SIZE), Image.LANCZOS)
...
#         if _is_blank(patch):
#             continue
#         roi_img = Image.fromarray(patch).resize(
...
class QueryTimeROIRetriever:
    """
    Retrieve full images, then extract ROIs using Grounding DINO
    with SUBQUERIES as prompts.
    """
    ...
    def __init__(
        ...
        # Load Grounding DINO if not provided
        if gdino_model is None:
            ...
            self.gdino_model.to("cuda" if torch.cuda.is_available() else "cpu")
        else:
            self.gdino_model = gdino_model
            self.gdino_processor = gdino_processor
    
    # def retrieve_images(self, query: str, k: int = 3) -> List[dict]:
    #     """Retrieve top-k full images using CLIP similarity."""
    #     # Get query embedding
    #     # inputs = self.clip_processor(text=[query], return_tensors="pt", padding=True)
    #     inputs = self.clip_processor(text=[query], return_tensors="pt", padding=True).to(next(self.clip_model.parameters()).device)
    #     with torch.no_grad():
    #         text_features = self.clip_model.get_text_features(**inputs)
    #         text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        
    #     query_emb = text_features.cpu().numpy().astype(np.float32)
    ...
    def retrieve_images(self, query: str, k: int = 3) -> List[dict]:
        ...
        with torch.no_grad():
            ...
            # Handle case where output is BaseModelOutputWithPooling vs raw tensor
            if isinstance(output, torch.Tensor):
                text_features = output
    ...
    def extract_rois_with_query(
        ...
        """
        Extract ROIs from a retrieved image using Grounding DINO
        with the QUERY as the prompt.
        """
        # Decode image
        img_data = base64.b64decode(image_metadata['image_base64'])
        image = Image.open(BytesIO(img_data)).convert('RGB')
        
        # Build DINO prompt from query
        # Clean up query for DINO (add period-separated format)
        query_words = query.lower().split()
        # Keep meaningful words
        stop_words = {'the', 'a', 'an', 'is', 'are', 'what', 'which', 'how', 'of', 'in', 'on', 'for'}
        meaningful = [w for w in query_words if w not in stop_words and len(w) > 2]
        
        # Create DINO prompt
        dino_prompt = " . ".join(meaningful[:10]) + " ."
        
        # Add domain defaults
        dino_prompt += " diagram . label . chart . map . region . object ."
        ...
        # Post-process without threshold args (not supported in this API version)
        ...
        # Extract ROIs - filter by score threshold manually
        rois = []
        boxes = results["boxes"]
        scores = results["scores"]
        labels = results.get("labels", ["object"] * len(boxes))
        ...
        if len(scores) > 0:
            ...
            if isinstance(labels, list):
                labels = [labels[i] for i in sorted_indices.tolist()]
            else:
                labels = [labels[i] for i in sorted_indices.tolist()]
        for i, (box, score) in enumerate(zip(boxes.tolist(), scores.tolist())):
            ...
            # Crop ROI
            roi_image = image.crop((x0, y0, x1, y1))
            ...
            rois.append(RoiInfo(
                roi_id=f"{image_metadata['image_id']}_roi_{len(rois)}",
                bbox=[x0, y0, x1, y1],
                source_image=image_metadata['source_path'],
                image_patch=roi_base64,
                caption=f"{label} (query: {query[:30]}...)",
                score=float(score),
                subject=image_metadata.get('subject', ''),
                topic=image_metadata.get('topic', ''),
            ))
    ...
    def retrieve(
        ...
        subquery: str,
        k_images: int = 2,
        max_rois_per_image: int = 3,
    ) -> List[RoiInfo]:
        """
        Full retrieval pipeline:
        1. Retrieve relevant full images
        2. Extract ROIs using subquery as DINO prompt
        """
        # Step 1: Retrieve images
        images = self.retrieve_images(subquery, k=k_images)
        
        # Step 2: Extract ROIs from each image using query
        all_rois = []
        for img_meta in images:
            rois = self.extract_rois_with_query(
                img_meta,
                query=subquery,
                max_rois=max_rois_per_image,
            )
            all_rois.extend(rois)
        
        # Sort by score
        all_rois.sort(key=lambda x: x.score, reverse=True)
        
        return all_rois[:5]  # Return top 5
...
#             # Tavily web search
#             try:
#                 tavily_docs = tavily_search(subquery, retrieval_k)
...
# ) -> State:
#     """Updated retriever using query-time ROI extraction with Grounding DINO."""
    
#     with tracer.start_as_current_span("Retriever", openinference_span_kind="retriever") as retriever_span:
        
#         # Initialize BM25 for text retrieval
#         if use_hybrid:
#             text_data = data[data['media_type'] == 'text']
#             corpus_texts = text_data["text"].tolist()
#             bm25_retriever = BM25Retriever(corpus_texts)
        
#         # Initialize query-time ROI retriever
#         roi_retriever = QueryTimeROIRetriever(
...
#                 else:
#                     q_text_embed = text_to_embedding(subquery).reshape(1, -1)
#                     D_text, I_text = text_index.search(q_text_embed.astype(np.float32), retrieval_k * 2)
#                     for j in range(retrieval_k * 2):
#                         idx = I_text[0][j]
#                         if 0 <= idx < len(data):
#                             doc = data.iloc[idx]["text"].strip()
...
#                             evidence.append(doc)
#             except Exception as e:
#                 print(f"Text retrieval failed: {subquery}\n{e}")
...
#             # IMAGE ROI RETRIEVAL - Query-time DINO!
#             image_rois = []
#             if image_metadata and len(image_metadata) > 0:
#                 try:
#                     raw_rois = roi_retriever.retrieve(
#                         subquery=subquery,
#                         k_images=2,
#                         max_rois_per_image=3,
#                     )
#                     # Move image patches into the shared cache to avoid
#                     # duplicating potentially large base64 blobs when the
#                     # same ROI is returned for multiple subqueries.
#                     for roi in raw_rois:
#                         if roi.image_patch and roi.roi_id not in state.image_patch_cache:
#                             state.image_patch_cache[roi.roi_id] = roi.image_patch
#                         roi.image_patch = ""  # clear from RoiInfo; resolve via cache
#                     image_rois = raw_rois
#                     print(f"  Query '{subquery[:30]}...' → {len(image_rois)} ROIs extracted")
#                 except Exception as e:
...
def retriever_step(
    ...
    data,
    tavily_search_fn,
    bm25_retriever_class,
    text_to_embedding_fn,
    rerank_with_cross_encoder_fn,
    k: int = 3,
    use_hybrid: bool = True,
    use_cross_encoder: bool = True,
) -> State:
    ...
    with tracer.start_as_current_span("Retriever", openinference_span_kind="retriever") as retriever_span:
        ...
        if use_hybrid:
            text_data = data[data['media_type'] == 'text']
            corpus_texts = text_data["text"].tolist()
            bm25_retriever = bm25_retriever_class(corpus_texts)
        ...
        for subquery in state.subqueries:
            ...
            try:
                if use_hybrid:
                    ...
                    q_text_embed = text_to_embedding_fn(subquery).reshape(1, -1)
                    D_text, I_text = text_index.search(q_text_embed.astype(np.float32), retrieval_k * 2)
                    for j in range(retrieval_k * 2):
                        idx = I_text[0][j]
                        if 0 <= idx < len(data):
            ...
            # WEB SEARCH
            try:
                tavily_docs = tavily_search_fn(subquery, retrieval_k)
                evidence.extend(tavily_docs)
            except Exception as e:
                print(f"Tavily search failed: {subquery}\n{e}")
            ...
            if use_cross_encoder and evidence:
                local_set = set(local_docs)
                web_evidence = [e for e in evidence if e not in local_set]
                reranked_web = rerank_with_cross_encoder_fn(subquery, web_evidence, top_k=max(1, retrieval_k - 1))
                # Keep top 2 local docs to ensure corpus representation
                best_local = local_docs[:2]
                seen = set(best_local)
                merged = list(best_local)
                for e in reranked_web:
                    if e not in seen:
            ...
            # NO ROI - just text evidence
            retrieved[subquery] = ChunkInfo(
                text_chunks=evidence,
                image_rois=[],  # Empty - no ROI extraction
            )
            
            print(f"  Query '{subquery[:40]}...' → {len(evidence)} text chunks")
        
        state.retrieved_chunks = retrieved
        
        # Log question images (they will be used by solver directly)
        num_question_images = len([p for p in (state.image_paths or []) if p and os.path.exists(p)])
        print(f"  Question has {num_question_images} images (passed directly to solver)")
...
# def run_prompt_sanity_check():
#     """Quick test of GDINO prompt resolution for all ScienceQA subject/topic combos."""
#     test_cases = [
