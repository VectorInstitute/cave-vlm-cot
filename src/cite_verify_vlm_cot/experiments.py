...
_parser.add_argument("--shard", type=int, default=None, help="0-based shard index to process")
_parser.add_argument("--num-shards", type=int, default=1, help="Total number of shards (default: 1 = no sharding)")
...
SHARD_INDEX = _args.shard if _args.shard is not None else (_slurm_task_id if _slurm_task_id >= 0 else 0)
...
# Your local modules
from utils import State, safe_parse_json, safe_str
...
def _index_needs_rebuild(expected_rows: int) -> bool:
    ...
    try:
        ...
        if idx.ntotal < expected_rows:
            print(f"Index has {idx.ntotal} vectors but dataset has {expected_rows} text rows — rebuilding.")
            return True
    except Exception:
        return True
    return False
...
if _needs_planner:
    ...
    print("1. Loading Qwen2.5-7B-Instruct for Planner (reverted from Qwen3-8B)...")
