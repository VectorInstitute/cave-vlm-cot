...
def format_evidence(state: State, image_descriptions: List[str]) -> Tuple[str, str]:
    ...
    """
    chunks = _build_text_chunk_list(state.retrieved_chunks, total_cap=15, truncate=400)
    text_evidence = [f"[Text Evidence {i + 1}]: {chunk}" for i, chunk in enumerate(chunks)]
...
def check_answer_consistency(
    ...
    pairs = [[reasoning, f"The correct answer is {CHOICE_LABELS[i]}: {choice}"] for i, choice in enumerate(choices)]
    ...
    if best_idx != stated_idx and best_score > stated_score + threshold:
        ...
        print(
            f"  [CONSISTENCY CHECK] Reasoning supports {best_letter} "
...
def parse_solver_output(state, full_output: str, choices: list, encoder=None):
    ...
    conclusion_match = re.search(r"<CONCLUSION>(.*?)</CONCLUSION>", full_output, re.IGNORECASE | re.DOTALL)
    ...
    reasoning_match = re.search(r"<REASONING>(.*?)</REASONING>", full_output, re.IGNORECASE | re.DOTALL)
    if reasoning_match:
        reasoning_parts.append(reasoning_match.group(1).strip())

    visual_match = re.search(
        r"<(?:CAPTION|OBSERVATIONS)>(.*?)</(?:CAPTION|OBSERVATIONS)>", full_output, re.IGNORECASE | re.DOTALL
    )
    if visual_match:
...
def solver_step(state: State, model, processor, kwargs) -> State:
    ...
    with tracer.start_as_current_span("Solver", openinference_span_kind="chain") as solver_span:
        ...
        with tracer.start_as_current_span("VLM-Inference", openinference_span_kind="llm") as vlm_span:
            ...
            print(
                f"  Citations: {len(text_cites)} text, {len(image_cites)} image "
                f"({'cited' if image_cites else 'not cited'})"
            )
            if images and not image_cites and not has_observations:
                print("  [Solver] Image passed but not cited — citation injector will handle.")
