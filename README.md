...
```
ScienceQA question + images
           │
    ┌──────▼──────┐
    │   Planner   │  Qwen2.5-7B (GPU 0)  →  up to 8 search queries
    └──────┬──────┘
           │
    ┌──────▼──────┐
    │  Retriever  │  Hybrid local (FAISS + BM25 + RRF) + DuckDuckGo web search
    └──────┬──────┘       cross-encoder reranking (GPU 0)
           │
    ┌──────▼──────┐
    │    Solver   │  Llama-3.2V-11B (GPU 1)  →  structured CoT with inline citations
    └──────┬──────┘
           │
    ┌──────▼──────────┐
    │ Citation Injector│  cross-encoder post-hoc citation matching
    └──────┬──────────┘
           │
    ┌──────▼──────┐
    │  Verifier   │  Qwen2.5-VL-7B (GPU 2)  →  VERIFIED / REJECTED + hallucination report
    └──────┬──────┘
           │
    ┌──────▼──────┐         ┌─────────────────────────┐
    │   Router    │─REJECTED─▶  back to Planner        │
    └──────┬──────┘  (≤3x)  │  with verifier feedback  │
     VERIFIED               └─────────────────────────┘
           │
    Final cited + verified answer
```
...
```
cite_verify_vlm_cot/
├── planner.py           # Stage 1: query generation
├── retriever.py         # Stage 2: hybrid retrieval + web search
├── solver.py            # Stage 3: multimodal CoT generation
├── citation_injector.py # Stage 4: post-hoc citation grounding
├── verifier.py          # Stage 5: hallucination detection + LangGraph graph
├── evaluations.py       # 27 evaluators including CaVeScore and AIS
├── experiments.py       # Phoenix experiment runner (sharding-aware)
├── utils.py             # State schema, index builders, helpers
├── tracer.py            # OpenTelemetry + Phoenix setup
├── data-preparation.py  # ScienceQA augmentation (captions, OCR, paths)
├── aggregate_shards.py  # Merge Phoenix CSV exports from multiple shards
├── cave_array.sh        # SLURM array job (3 shards × 3 GPUs)
└── scienceqa_augmented.csv  # Generated by data-preparation.py
```
